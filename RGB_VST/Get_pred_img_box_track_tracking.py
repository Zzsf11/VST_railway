        ...
        for i, frame in tqdm.tqdm(enumerate(diff_frames)):
            ...
            if detections is not None:
                ...
                for t in tracker.removed_stracks:
                    #TODO: 这个参数很重要
                    if t.end_frame - t.start_frame >= 3:
                        tlwh = t.tlwh
                        tid = t.track_id
                        # 检查是否已经存在相同的bbox和id
                ...
                combined_img = cv2.addWeighted(mask_4_vis, alpha, pred, 1 - alpha, 0, result_img)
                combined_img = cv2.addWeighted(mask_4_vis, alpha, result_img, 1 - alpha, 0, result_img)
